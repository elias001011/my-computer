...
const EN_TEXT = new Map([
  ...
  ['Use este provider para Minimax, Together, Fireworks, Alibaba Model Studio (Qwen), Moonshot AI (Kimi), Zhipu/Z.ai (GLM), servidores próprios ou qualquer API que aceite o formato /v1/chat/completions. Base URLs conhecidas: Alibaba Model Studio (internacional) https://dashscope-intl.aliyuncs.com/compatible-mode/v1 — Moonshot AI https://api.moonshot.ai/v1 (ou .cn na China) — Zhipu/Z.ai https://api.z.ai/api/paas/v4/ (ou open.bigmodel.cn na China). Depois de configurar a Base URL e a key, o app tenta descobrir os modelos via /models; se não vier nada, use "Modelo personalizado" e digite o ID exato do modelo (ex.: qwen-max, kimi-k2.5, glm-4.6).', 'Use this provider for Minimax, Together, Fireworks, Alibaba Model Studio (Qwen), Moonshot AI (Kimi), Zhipu/Z.ai (GLM), self-hosted servers, or any API that accepts the /v1/chat/completions format. Known base URLs: Alibaba Model Studio (international) https://dashscope-intl.aliyuncs.com/compatible-mode/v1 — Moonshot AI https://api.moonshot.ai/v1 (or .cn in China) — Zhipu/Z.ai https://api.z.ai/api/paas/v4/ (or open.bigmodel.cn in China). After setting the base URL and key, the app tries to discover models via /models; if none show up, use "Custom model" and type the exact model id (e.g. qwen-max, kimi-k2.5, glm-4.6).'],
  ...
  ['Quando ligado, instrui a IA a nunca responder de suposição quando uma tool pode confirmar, a encadear várias rodadas até entender o mecanismo (não só até ter algo plausível pra dizer), e a preferir sessão de terminal em vez de um comando único quando a tarefa tiver múltiplas etapas. Ajuda modelos sem raciocínio estendido (instant) a saberem que devem investigar antes de escrever a resposta.', 'When enabled, instructs the AI to never answer from assumption when a tool could confirm it, to chain multiple rounds until it actually understands the mechanism (not just until it has something plausible to say), and to prefer a terminal session over a single one-shot command when the task has multiple steps. Helps models without extended reasoning (instant) know they must investigate before writing the answer.'],
          ...
          <div class="modal-body settings-layout">
            ...
            <section class="modal-section settings-panel ${activeSection === 'providers' ? 'active' : ''}" data-section="providers">
              ...
              ${
                apiProviderInfo.id === 'openai-compatible'
                  ? '<p class="help-text">Use este provider para Minimax, Together, Fireworks, Alibaba Model Studio (Qwen), Moonshot AI (Kimi), Zhipu/Z.ai (GLM), servidores próprios ou qualquer API que aceite o formato /v1/chat/completions. Base URLs conhecidas: Alibaba Model Studio (internacional) https://dashscope-intl.aliyuncs.com/compatible-mode/v1 — Moonshot AI https://api.moonshot.ai/v1 (ou .cn na China) — Zhipu/Z.ai https://api.z.ai/api/paas/v4/ (ou open.bigmodel.cn na China). Depois de configurar a Base URL e a key, o app tenta descobrir os modelos via /models; se não vier nada, use "Modelo personalizado" e digite o ID exato do modelo (ex.: qwen-max, kimi-k2.5, glm-4.6).</p>'
                  : ''
              }
            </section>
            ...
            <section class="modal-section settings-panel ${activeSection === 'tools' ? 'active' : ''}" data-section="tools">
              ...
              <div class="toggle-list">
                ${renderToolToggle('deepInvestigation', 'Incentivar a IA a fazer investigações mais profundas', 'Quando ligado, instrui a IA a nunca responder de suposição quando uma tool pode confirmar, a encadear várias rodadas até entender o mecanismo (não só até ter algo plausível pra dizer), e a preferir sessão de terminal em vez de um comando único quando a tarefa tiver múltiplas etapas. Ajuda modelos sem raciocínio estendido (instant) a saberem que devem investigar antes de escrever a resposta.')}
